...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mData = Arrays.asList(getResources()
                .getStringArray(idData == 0 ? R.array.simple_data : idData));
    ...
    public void setSelectedItemPosition(int position, final boolean animated) {
      ...
      boolean isComponentDrawn = mItemHeight > 0;
        if (animated && isComponentDrawn && mScroller.isFinished()) { // We go non-animated regardless of "animated" parameter if scroller is in motion
